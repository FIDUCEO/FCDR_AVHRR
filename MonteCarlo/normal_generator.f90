MODULE normal_generator

! Note this has been modified to use a more modern generator at the bit level
! see f90 version on 
!
! http://www.math.sci.hiroshima-u.ac.jp/~m-mat/MT/SFMT/VERSIONS/versions.html
!
! and the double precision 0-1 generator replaces the explicit code
!
USE SFMT

PRIVATE
PUBLIC :: normal_random
PUBLIC :: uniform_random
PUBLIC :: normal_random_double
PUBLIC :: uniform_random_double
...
SUBROUTINE Set_Random_Seed(oseed)

  INTEGER, INTENT(OUT) :: oseed
  ...
  !
  oseed = this_seed
  CALL init_gen_rand(this_seed)

END SUBROUTINE Set_Random_Seed

REAL FUNCTION normal_random(mean,stdev)

  REAL, INTENT(IN) :: mean
  REAL, INTENT(IN) :: stdev

  normal_random = r4_normal_ab(mean,stdev,this_seed)

END FUNCTION normal_random
REAL FUNCTION normal_random_double(mean,stdev)

  REAL(kind = 8), INTENT(IN) :: mean
  REAL(kind = 8), INTENT(IN) :: stdev

  normal_random_double = r8_normal_ab(mean,stdev,this_seed)

END FUNCTION normal_random_double

REAL FUNCTION uniform_random()

  uniform_random = r4_uniform_01(this_seed)

END FUNCTION uniform_random
REAL FUNCTION uniform_random_double()

  uniform_random_double = r8_uniform_01(this_seed)

END FUNCTION uniform_random_double
...
!
  ...
  r4_uniform_01 = REAL(genrand_res53())
